package com.licker2689.lic.functions;
...
import static com.licker2689.lic.ItemCommand.prefix;

@SuppressWarnings("all")
public class LICFunction {
    ...
    public static void makeCooldown(Player p, float cooldown, CommandAction ca) {
        if (isHoldItem(p)) {
            if (!NBT.hasTagKey(p.getItemInHand(), "lic_uuid")) {
                p.setItemInHand(NBT.setStringTag(p.getItemInHand(), "lic_uuid", UUID.randomUUID().toString()));
            }
    ...
    public static boolean isCooldown(Player p, ItemStack item, CommandAction ca) {
        ...
        if (!plugin.cooldown.get(p.getUniqueId()).containsKey(UUID.fromString(NBT.getStringTag(item, "lic_uuid")))) return false;
        if (!plugin.cooldown.get(p.getUniqueId()).get(UUID.fromString(NBT.getStringTag(item, "lic_uuid"))).containsKey(ca)){
            return false;
        }else{
            return true;
        }
    ...
    public static void cooldown(Player p, CommandAction ca, int cooldown) {
        ...
        UUID uuid = null;
        try {
            uuid = UUID.fromString(NBT.getStringTag(p.getItemInHand(), "lic_uuid"));
